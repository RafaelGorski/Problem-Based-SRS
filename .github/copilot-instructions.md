...
| `canvas-asset-drift` | error | A downloaded asset in the advertised canvas release fails the structural archive contract. | Inspect the named asset and failed gate, repair the packager/workflow, and publish a corrected canvas release. |
| `canvas-asset-mismatch` | error | The zip and tar.gz assets in the current canvas release contain different normalized file paths. | Rebuild both assets from the same staged tree and republish; do not treat format metadata or directory entries as mismatches. |
