...
export function summarize({
  ...
  const cuttable = dangling.filter((l) => !unpublishable.includes(l));
  ...
  const strandedTags = new Set(stranded.map((s) => s.tag));
  const strandedTrains = new Set(stranded.map((s) => s.train));
  const pending = cuttable.filter((l) => !strandedTags.has(l.tag));
